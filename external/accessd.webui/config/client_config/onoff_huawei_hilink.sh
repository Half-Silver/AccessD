...
# include the accessd helper functions
source /usr/local/sbin/accessd_helpers.sh
...
if [ -z "$hilink_password" ] || [ -z "$hilink_pin" ]; then 
    _getAuthRouter
    if [ ! -z "$accessd_user" ]; then hilink_user="$accessd_user"; fi
    if [ ! -z "$accessd_password" ]; then hilink_password="$accessd_password"; fi
    if [ ! -z "$accessd_pin" ]; then hilink_pin="$accessd_pin"; fi
fi
