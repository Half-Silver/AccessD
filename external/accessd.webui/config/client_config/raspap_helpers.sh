#!/bin/bash
# 
# Helper functions to extract informations from AccessD config/settings
#
# zbchristian 2021
#
# get the values of a AccessD config variable
# call: _getRaspapConfig RASPAP_MOBILEDATA_CONFIG

accessd_webroot="/var/www/html"

function _getWebRoot() {
    local path
    path=$(cat /etc/lighttpd/lighttpd.conf | sed -rn "s/server.document-root \s*= \"([^ \s]*)\"/\1/p")
    if [ ! -z "$path" ]; then accessd_webroot="$path"; fi
    if [ -z "$path" ]; then return 1; else return 0; fi
}

# expand an AccessD config variable utilizing PHP
function _getRaspapConfig() {
    ...
    if [ ! -z "$var" ]; then 
        ...
        conf="$accessd_webroot/includes/config.php"
        if [ -f "$conf" ]; then
            conf=$(php -r 'include "'$conf'"; echo '$var';' 2> /dev/null)
            if [ ! -z "$conf" ] && [ -d ${conf%/*} ]; then accessd_config="$conf"; fi
        fi
    fi
    if [ -z "$accessd_config" ]; then return 1; else return 0; fi
}
...
function _getAuthRouter() {
    ...
    mfile="$accessd_config" 
    if [ -f $mfile ]; then      
        mdata=$(cat "$mfile")
        pin=$(echo "$mdata" | sed -rn 's/pin = ([^ \s]*)/\1/ip')
        if [ ! -z "$pin" ]; then accessd_pin="$pin"; fi
        user=$(echo "$mdata" | sed -rn 's/router_user = ([^ \s]*)/\1/ip')
        if [ ! -z "$user" ]; then accessd_user="$user"; fi
        pw=$(echo "$mdata" | sed -rn 's/router_pw = ([^ \s]*)/\1/ip')
        if [ ! -z "$pw" ]; then accessd_password="$pw"; fi
        return 0
    fi
    return 1
}
