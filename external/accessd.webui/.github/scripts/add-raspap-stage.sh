...
mkdir -p stage-accessd/package-accessd
...
curl -sL https://install.accessd.com | bash -s -- \\
...
EOF
  ...
  INSTALLER_COMMANDS=$(
    cat <<-EOF
curl -sL https://install.accessd.com | bash -s -- --yes --openvpn 1 --restapi 1 --adblock 1 --wireguard 1 --tcp-bbr 1 --check 0
EOF
  )
fi

cat >stage-accessd/prerun.sh <<-EOF
#!/bin/bash -e
...
cat >stage-accessd/package-accessd/00-run-chroot.sh <<-EOF
#!/bin/bash -e
...
# Fetch AccessD version and set MOTD
RASPAP_VERSION=\$(curl -sL https://install.raspap.com | bash -s -- --version)
echo "\$RASPAP_VERSION" | tee /etc/motd
EOF

chmod +x stage-accessd/prerun.sh
chmod +x stage-accessd/package-accessd/00-run-chroot.sh
