...
NAME=accessdd
DESC="Service control for AccessD"
CONFIGFILE="/etc/accessd/hostapd.ini"
DAEMONPATH="/lib/systemd/system/accessdd.service"
...
systemctl stop 'accessd-network-activity@*.service'
...
if [ -f "$DAEMONPATH" ] && [ -n "$interface" ]; then
    echo "Changing AccessD Daemon --interface to $interface"
...
echo "Starting accessd-network-activity@${interface}.service"
systemctl start accessd-network-activity@${interface}.service
...
echo "AccessD service start DONE"
