#!/bin/bash
#
# AccessD OS Subitem Generator for RPi Imager 2.x
...
# Set defaults
NAME="AccessD"
WEBSITE="https://accessd.com/"
ICON="https://accessd.com/assets/images/raspAP-logo.svg"
DESCRIPTION="Turn your Raspberry Pi into a full-featured wireless router."
REPO="AccessD/accessd-webui"
FILENAME="os-subitem.json"

# Fetch latest release from GitHub (adapted from AccessD installer)
_get_release() {
    local response
...
BASE_URL="https://github.com/AccessD/accessd-webui/releases/download/${VERSION}"
...
ARMHF_ZIP="accessd-trixie-armhf-lite-${VERSION}.img.zip"

if [ -f "$ARMHF_ZIP" ]; then
    ARMHF_IMG=$(ls *-accessd-trixie-armhf-lite-${VERSION}.img 2>/dev/null | head -1)
...
ARM64_ZIP="accessd-trixie-arm64-lite-${VERSION}.img.zip"

if [ -f "$ARM64_ZIP" ]; then
    ARM64_IMG=$(ls *-accessd-trixie-arm64-lite-${VERSION}.img 2>/dev/null | head -1)
