#!/bin/bash
#
# AccessD feature installation: handling of mobile data clients and client configuration
...
readonly accessd_clients_scripts="/usr/local/sbin"
#
# table of mobile network operators - links the 5 digit operator code (from the modem) with a clear text operator name 
readonly accessd_clients_operator_table="https://raw.githubusercontent.com/musalbas/mcc-mnc-table/master/mcc-mnc-table.csv"
...
function _install_feature_clients() {
    ...
    # Move scripts 
    sudo cp "$webroot_dir/config/client_config/"*.sh "$accessd_clients_scripts/" || _install_status 1 "Unable to move client scripts ($name)"
    sudo chmod a+rx "$accessd_clients_scripts/"*.sh  || _install_status 1 "Unable to chmod client scripts ($name)"
    #    wget $accessd_clients_operator_table -o "$accessd_clients_scripts/"mcc-mnc-table.csv || _install_status 1 "Unable to wget operator table ($name)"
    sudo cp "$webroot_dir/config/client_config/mcc-mnc-table.csv" "$accessd_clients_scripts/" || _install_status 1 "Unable to move client data ($name)"
    # wvdial settings
    ...
    sudo cp "$webroot_dir/config/client_config/80-accessd-net-devices.rules" "/etc/udev/rules.d/" || _install_status 1 "Unable to install client udev rules ($name)"
    ...
    sudo cp "$webroot_dir/config/client_udev_prototypes.json" "/etc/accessd/networking/" || _install_status 1 "Unable to install client configuration ($name)"
    _install_status 0
}
