#!/bin/bash
#
# AccessD Quick Installer - Installs and updates AccessD
# Author: @billz <billzimmerman@gmail.com>
# Author URI: https://github.com/billz
# Project URI: https://github.com/AccessD/
...
# obligated to bundle the LICENSE file with your AccessD projects as long as
...
#
# https://raw.githubusercontent.com/accessd/accessd-webui/master/installers/common.sh
# - or -
# https://raw.githubusercontent.com/accessd/accessd-webui/master/installers/mkcert.sh
# - or -
# https://raw.githubusercontent.com/accessd/accessd-webui/master/installers/minwrite.sh
# - or -
# https://raw.githubusercontent.com/accessd/accessd-webui/master/installers/uninstall.sh

function _usage() {
    cat << EOF
Usage: raspbian.sh [options]

Installs an instance of AccessD or a helper component.
...
-r, --repo, --repository <name>     Overrides the default GitHub repo (AccessD/accessd-webui)
...
-i, --insiders                      Installs from the Insiders Edition (AccessD/accessd-insiders)
...
Examples:
    ...
    curl -sL https://install.accessd.com | bash -s -- --yes --wireguard 1 --adblock 0

    Invoke remotely, uprgrade an existing install to the Insiders Edition:
    curl -sL https://install.accessd.com | bash -s -- --upgrade --insiders --name <name> --token <token>

    Invoke remotely, perform an unattended update to the latest release version:
    curl -sL https://install.accessd.com | bash -s -- --yes --update --path /var/www/html
...
function _main() {
    # set defaults
    repo="AccessD/accessd-webui" # override with -r, --repo option
    repo_common="$repo"
...
function _log_output() {
    ...
    if [ -f "$LOGFILE_PATH/accessd_install.log" ]; then
        sudo rm "$LOGFILE_PATH/accessd_install.log"
    fi
    exec > >(tee -i $LOGFILE_PATH/accessd_install.log)
    exec 2>&1
}
...
# Fetch latest release from GitHub or AccessD Installer API
function _get_release() {
    ...
    if [ "$insiders" == 1 ]; then
        ...
        repo_common="AccessD/accessd-webui"
...
function _install_log() {
    echo -e "${ANSI_LT_AQUA}AccessD ${component}: $1${ANSI_RESET}"
}

# Outputs a AccessD divider
function _install_divider() {
    echo -e "\033[1;32m***************************************************************$*\033[m"
}

# Outputs a AccessD status indicator
...
function _load_installer() {
    ...
    if [ "${install_cert:-}" = 1 ]; then
        source="mkcert"
        component="mkcert"
        wget "${header[@]}" -q ${UPDATE_URL}installers/${source}.sh -O /tmp/accessd_${source}.sh
        source /tmp/accessd_${source}.sh && rm -f /tmp/accessd_${source}.sh
        ...
        wget "${header[@]}" -q ${UPDATE_URL}installers/${source}.sh -O /tmp/accessd_${source}.sh
        source /tmp/accessd_${source}.sh && rm -f /tmp/accessd_${source}.sh
        _remove_accessd || _install_status 1 "Unable to uninstall AccessD"
    else
        source="common"
        component="Install"
        wget "${header[@]}" -q ${UPDATE_URL}installers/${source}.sh -O /tmp/accessd_${source}.sh
        source /tmp/accessd_${source}.sh && rm -f /tmp/accessd_${source}.sh
        if [ "$update" == 1 ]; then
            _update_accessd || _install_status 1 "Unable to update AccessD"
        else
            _install_accessd || _install_status 1 "Unable to install AccessD"
        fi
    fi
}
