#!/bin/bash
#
# AccessD installation functions
...
readonly accessd_network="$accessd_dir/networking/"
readonly accessd_router="/etc/lighttpd/conf-available/50-accessd-router.conf"
...
function _install_accessd() {
    ...
    _create_accessd_directories
    _check_for_old_configs
    _optimize_php
    _download_latest_files
    _change_file_ownership
    _create_hostapd_scripts
    _install_accessd_hostapd
...
# The user is not prompted to install new AccessD components.
# The -y, --yes and -p, --path switches may be used for an unattended update.
function _update_accessd() {
    _display_welcome
    _config_installation
    _update_system_packages
    _install_dependencies
...
function _config_installation() {
    ...
    echo "Configuration directory: ${accessd_dir}"
...
function _check_notify_ubuntu() {
    if [ ${OS,,} = "ubuntu" ]; then
        _install_status 2 "Ubuntu Server requires manual pre- and post-install steps. See https://docs.accessd.com/manual/"
...
# Verifies existence and permissions of AccessD directory
function _create_accessd_directories() {
    _install_log "Creating AccessD directories"
    if [ -d "$accessd_dir" ]; then
        sudo mv $accessd_dir "$accessd_dir.`date +%F-%R`" || _install_status 1 "Unable to move old '$accessd_dir' out of the way"
    fi
    sudo mkdir -p "$accessd_dir" || _install_status 1 "Unable to create directory '$accessd_dir'"

    # Create a directory for existing file backups.
    sudo mkdir -p "$accessd_dir/backups"

    # Create a directory to store networking configs
    echo "Creating $accessd_dir/networking"
    sudo mkdir -p "$accessd_dir/networking"
    echo "Changing file ownership of $accessd_dir"
    sudo chown -R $accessd_user:$accessd_user "$accessd_dir" || _install_status 1 "Unable to change file ownership for '$accessd_dir'"
}
...
function _create_hostapd_scripts() {
    ...
    sudo mkdir $accessd_dir/hostapd || _install_status 1 "Unable to create directory '$accessd_dir/hostapd'"

    # Copy service control shell scripts
    sudo cp "$webroot_dir/installers/"servicestart.sh "$accessd_dir/hostapd" || _install_status 1 "Unable to move service control scripts"
    # Change ownership and permissions of hostapd control scripts
    sudo chown -c root:root "$accessd_dir/hostapd/"*.sh || _install_status 1 "Unable change owner and/or group"
    sudo chmod 750 "$accessd_dir/hostapd/"*.sh || _install_status 1 "Unable to change file permissions"
    _install_status 0
}
...
function _create_plugin_scripts() {
    ...
    sudo mkdir -p $accessd_dir/plugins || _install_status 1 "Unable to create directory '$accessd_dir/plugins'"

    # Copy plugin helper script
    sudo cp "$webroot_dir/installers/"plugin_helper.sh "$accessd_dir/plugins" || _install_status 1 "Unable to move plugin script"
    # Change ownership and permissions of plugin script
    sudo chown -c root:root "$accessd_dir/plugins/"*.sh || _install_status 1 "Unable change owner and/or group"
    sudo chmod 750 "$accessd_dir/plugins/"*.sh || _install_status 1 "Unable to change file permissions"
    _install_status 0
}
...
function _create_lighttpd_scripts() {
    ...
    sudo mkdir $accessd_dir/lighttpd || _install_status 1 "Unable to create directory '$accessd_dir/lighttpd"

    # Copy service control shell scripts
    echo "Copying configport.sh to $accessd_dir/lighttpd"
    sudo cp "$webroot_dir/installers/"configport.sh "$accessd_dir/lighttpd" || _install_status 1 "Unable to move service control scripts"
    # Change ownership and permissions of lighttpd scripts
    echo "Changing file ownership"
    sudo chown -c root:root "$accessd_dir/lighttpd/"*.sh || _install_status 1 "Unable change owner and/or group"
    sudo chmod 750 "$accessd_dir/lighttpd/"*.sh || _install_status 1 "Unable to change file permissions"
    _install_status 0
}
...
function _install_lighttpd_configs() {
    ...
    # Copy config files
    echo "Copying 50-accessd-router.conf to /etc/lighttpd/conf-available"

    CONFSRC="$webroot_dir/config/50-accessd-router.conf"
    ...
    # Substitute values
    awk "{gsub(\"/REPLACE_ME\",\"$HTROOT\")}1" $CONFSRC > /tmp/50-accessd-router.conf

    # Copy into place
    sudo cp /tmp/50-accessd-router.conf /etc/lighttpd/conf-available/ || _install_status 1 "Unable to copy lighttpd config file into place."

    # Link into conf-enabled
    echo "Creating link to /etc/lighttpd/conf-enabled"
    if ! [ -L $accessd_router ]; then
        echo "Existing 50-accessd-router.conf found. Unlinking."
        sudo unlink "/etc/lighttpd/conf-enabled/50-accessd-router.conf"
    fi
    echo "Linking 50-accessd-router.conf to /etc/lighttpd/conf-enabled/"
    sudo ln -s "/etc/lighttpd/conf-available/50-accessd-router.conf" "/etc/lighttpd/conf-enabled/50-accessd-router.conf" || _install_status 1 "Unable to symlink lighttpd config file (this is normal if the link already exists)."
    sudo systemctl restart lighttpd.service || _install_status 1 "Unable to restart lighttpd"
    _install_status 0
}
...
function _install_adblock() {
    ...
    if [ ! -d "$accessd_dir/adblock" ]; then
        echo "Creating $accessd_dir/adblock"
        sudo mkdir -p "$accessd_dir/adblock"
    fi
    ...
    echo "Adding blocklists to $accessd_dir/adblock"
    sudo cp /tmp/hostnames.txt $accessd_dir/adblock || _install_status 1 "Unable to move notracking hostnames"
    sudo cp /tmp/domains.txt $accessd_dir/adblock || _install_status 1 "Unable to move notracking domains"

    echo "Moving and setting permissions for blocklist update script"
    sudo cp "$webroot_dir/installers/"update_blocklist.sh "$accessd_dir/adblock" || _install_status 1 "Unable to move blocklist update script"

    # Make blocklists writable by www-data group, restrict update scripts to root
    sudo chown -c root:"$accessd_user" "$accessd_dir/adblock/"*.txt || _install_status 1 "Unable to change owner/group"
    sudo chown -c root:root "$accessd_dir/adblock/"*.sh || _install_status 1 "Unable to change owner/group"
    sudo chmod 750 "$accessd_dir/adblock/"*.sh || install_error "Unable to change file permissions"
    ...
    if [ ! -f "$accessd_adblock" ]; then
        ...
        sudo touch "$accessd_adblock"
        echo "conf-file=$accessd_dir/adblock/domains.txt" | sudo tee -a "$accessd_adblock" > /dev/null || _install_status 1 "Unable to write to $accessd_adblock"
        echo "addn-hosts=$accessd_dir/adblock/hostnames.txt" | sudo tee -a "$accessd_adblock" > /dev/null || _install_status 1 "Unable to write to $accessd_adblock"
    fi
    ...
    sudo sed -i '/dhcp-option=6/d' $accessd_wlan0 || _install_status 1 "Unable to modify $accessd_dnsmasq"
...
function _install_provider() {
    ...
    if ! [ "$skip" ]; then
        ...
        bin_path=${selected#*|}
        if ! grep -q "$bin_path" "$webroot_dir/installers/accessd.sudoers"; then
            echo "Adding $bin_path to accessd.sudoers"
            echo "www-data ALL=(ALL) NOPASSWD:$bin_path *" | sudo tee -a "$webroot_dir/installers/accessd.sudoers" > /dev/null || _install_status 1 "Unable to modify accessd.sudoers"
        fi
        ...
        echo "Adding VPN provider to $accessd_dir/provider.ini"
        if [ ! -f "$accessd_dir/provider.ini" ]; then
            sudo touch "$accessd_dir/provider.ini"
            echo "providerID = $answer" | sudo tee "$accessd_dir/provider.ini" > /dev/null || _install_status 1 "Unable to create $accessd_dir/provider.ini"
        elif ! grep -q "providerID = $answer" "$accessd_dir/provider.ini"; then
            echo "providerID = $answer" | sudo tee "$accessd_dir/provider.ini" > /dev/null || _install_status 1 "Unable to write to $accessd_dir/provider.ini"
        fi
    fi    
    _install_status 0
}
...
function _create_openvpn_scripts() {
    ...
    sudo mkdir $accessd_dir/openvpn || _install_status 1 "Unable to create directory '$accessd_dir/openvpn'"

    # Copy service auth control and logging scripts
    sudo cp "$webroot_dir/installers/"configauth.sh "$accessd_dir/openvpn" || _install_status 1 "Unable to move auth control script"
    sudo cp "$webroot_dir/installers/"openvpnlog.sh "$accessd_dir/openvpn" || _install_status 1 "Unable to move logging script"
    # Restrict script execution to root user
    sudo chown -c root:root "$accessd_dir/openvpn/"*.sh || _install_status 1 "Unable change owner and/or group"
    sudo chmod 750 "$accessd_dir/openvpn/"*.sh || _install_status 1 "Unable to change file permissions"
...
function _install_restapi() {
    ...
    sudo mv "$webroot_dir/api" "$accessd_dir/api"  || _install_status 1 "Unable to move api folder"
    ...
    python3 -m pip install -r "$accessd_dir/api/requirements.txt" --break-system-packages || _install_status 1 " Unable to install pip modules"
...
function _download_latest_files() {
    ...
    if [ "$insiders" == 1 ] && [ "$ssh" != 1 ] && [[ -z "$username"  ||  -z "$acctoken" ]]; then
        _install_status 3
        _install_status 0 "Insiders please read this: https://docs.accessd.com/insiders/#authentication"
    fi
    ...
    if [ -d "$webroot_dir" ] && [ "$update" == 0 ]; then
        ...
        if [ -d "$webroot_dir/plugins" ]; then
            sudo cp -r "$webroot_dir/plugins" "/tmp/accessd-user-plugins"
        fi
    ...
    if [ "$upgrade" == 1 ] || [ "$update" == 1 ]; then
        if [ -d "/tmp/accessd-user-plugins" ]; then
            sudo find /tmp/accessd-user-plugins -mindepth 1 -maxdepth 1 -type d -exec cp -r {} "$webroot_dir/plugins/" \; 2>/dev/null
            sudo rm -rf "/tmp/accessd-user-plugins"
        fi
    fi
    ...
    if [ "$update" == 1 ]; then
        ...
        if [ -f /tmp/accessd.auth ]; then
            _install_log "Applying existing authentication file to ${accessd_dir}"
            sudo mv /tmp/accessd.auth $accessd_dir || _install_status 1 "Unable to restore authentification credentials file to ${accessd_dir}"
        fi
    else
        echo "Copying primary AccessD config to ${webroot_dir}/includes/config.php"
...
function _change_file_ownership() {
    ...
    sudo chown -R $accessd_user:$accessd_user "$webroot_dir" || _install_status 1 "Unable to change file ownership for '$webroot_dir'"
}
...
function _check_for_old_configs() {
    if [ "$update" == 1 ]; then
       ...
       if [ -f $accessd_dir/accessd.auth ]; then
            _install_log "Moving existing accessd.auth file to /tmp"
            sudo mv $accessd_dir/accessd.auth /tmp || _install_status 1 "Unable to backup accessd.auth to /tmp"
        fi
    else
        _install_log "Checking for existing configs"
        if [ -f /etc/network/interfaces ]; then
            sudo cp /etc/network/interfaces "$accessd_dir/backups/interfaces.`date +%F-%R`"
            sudo ln -sf "$accessd_dir/backups/interfaces.`date +%F-%R`" "$accessd_dir/backups/interfaces"
        fi

        if [ -f /etc/hostapd/hostapd.conf ]; then
            sudo cp /etc/hostapd/hostapd.conf "$accessd_dir/backups/hostapd.conf.`date +%F-%R`"
            sudo ln -sf "$accessd_dir/backups/hostapd.conf.`date +%F-%R`" "$accessd_dir/backups/hostapd.conf"
        fi

        if [ -f $accessd_default ]; then
            sudo cp $accessd_default "$accessd_dir/backups/090_accessd.conf.`date +%F-%R`"
            sudo ln -sf "$accessd_dir/backups/090_accessd.conf.`date +%F-%R`" "$accessd_dir/backups/090_accessd.conf"
        fi

        if [ -f $accessd_wlan0 ]; then
            sudo cp $accessd_wlan0 "$accessd_dir/backups/090_wlan0.conf.`date +%F-%R`"
            sudo ln -sf "$accessd_dir/backups/090_wlan0.conf.`date +%F-%R`" "$accessd_dir/backups/090_wlan0.conf"
        fi

        if [ -f /etc/dhcpcd.conf ]; then
            sudo cp /etc/dhcpcd.conf "$accessd_dir/backups/dhcpcd.conf.`date +%F-%R`"
            sudo ln -sf "$accessd_dir/backups/dhcpcd.conf.`date +%F-%R`" "$accessd_dir/backups/dhcpcd.conf"
        fi

        for file in /etc/systemd/network/accessd-*.net*; do
            if [ -f "${file}" ]; then
                filename=$(basename $file)
                sudo cp "$file" "${accessd_dir}/backups/${filename}.`date +%F-%R`"
                sudo ln -sf "${accessd_dir}/backups/${filename}.`date +%F-%R`" "${accessd_dir}/backups/${filename}"
            fi
        done
    fi
    _install_status 0
...
function _default_configuration() {
    if [ "$upgrade" == 0 ]; then
        ...
        echo "Copying config/090_accessd.conf to $accessd_default"
        sudo cp $webroot_dir/config/090_accessd.conf $accessd_default || _install_status 1 "Unable to move dnsmasq default configuration file"

        echo "Copying config/090_wlan0.conf to $accessd_wlan0"
        sudo cp $webroot_dir/config/090_wlan0.conf $accessd_wlan0 || _install_status 1 "Unable to move dnsmasq wlan0 configuration file"
        ...
        echo "Copying config/defaults.json to $accessd_network"
        sudo cp $webroot_dir/config/defaults.json $accessd_network || _install_status 1 "Unable to move defaults.json settings"

        echo "Changing file ownership of ${accessd_network}defaults.json"
        sudo chown $accessd_user:$accessd_user "$accessd_network"defaults.json || _install_status 1 "Unable to change file ownership for defaults.json"
        ...
        if [ ${OS,,} = "ubuntu" ] && [[ ${RELEASE} =~ ^(22.04|20.04|19.10|18.04) ]]; then
            ...
            sudo cp $webroot_dir/config/accessd-bridge-br0.netplan /etc/netplan/accessd-bridge-br0.netplan || _install_status 1 "Unable to move br0 netplan file"
        else
            echo "Copying bridged AP config to /etc/systemd/network"
            sudo cp $webroot_dir/config/accessd-bridge-br0.netdev /etc/systemd/network/accessd-bridge-br0.netdev || _install_status 1 "Unable to move br0 netdev file"
            sudo cp $webroot_dir/config/accessd-br0-member-eth0.network /etc/systemd/network/accessd-br0-member-eth0.network || _install_status 1 "Unable to move br0 member file"
        fi
        ...
        _install_log "Copying defaults.json to $accessd_network"
        sudo cp $webroot_dir/config/defaults.json $accessd_network || _install_status 1 "Unable to move defaults.json settings"
    fi
}

# Install and enable AccessD daemon
function _enable_accessd_daemon() {
    _install_log "Enabling AccessD daemon"
    echo "Disable with: sudo systemctl disable accessdd.service"
    sudo cp $webroot_dir/installers/accessdd.service /lib/systemd/system/ || _install_status 1 "Unable to move accessd.service file"
    sudo systemctl daemon-reload
    sudo systemctl enable accessdd.service || _install_status 1 "Failed to enable accessd.service"
}

# Install hostapd@.service
function _install_accessd_hostapd() {
    _install_log "Installing AccessD hostapd@.service"
    sudo cp $webroot_dir/installers/hostapd@.service /etc/systemd/system/ || _install_status 1 "Unable to copy hostapd@.service file"
    sudo systemctl daemon-reload
    _install_status 0
}

# Configure IP forwarding, set IP tables rules, prompt to install AccessD daemon
function _configure_networking() {
    _install_log "Configuring networking"
    echo "Enabling IP forwarding"
    echo "net.ipv4.ip_forward=1" | sudo tee $accessd_sysctl > /dev/null || _install_status 1 "Unable to set IP forwarding"
    sudo sysctl -p $accessd_sysctl || _install_status 1 "Unable to execute sysctl"
    ...
    # Prompt to install AccessD daemon
    echo -n "Enable AccessD control service (Recommended)? [Y/n]: "
    if [ "$assume_yes" == 0 ]; then
        read answer < /dev/tty
        if [ "$answer" != "${answer#[Nn]}" ]; then
            _install_status 0 "(Skipped)"
        else
            _enable_accessd_daemon
        fi
    else
        echo -e
        _enable_accessd_daemon
    ...
    # Enable AccessD network activity monitor
    _enable_network_activity_monitor

    _install_status 0
 }

# Install and enable AccessD network activity monitor
function _enable_network_activity_monitor() {
    _install_log "Enabling AccessD network activity monitor"
    echo "Compiling accessd-network-monitor.c to /usr/local/bin/"
    if ! command -v gcc >/dev/null 2>&1; then
        echo "gcc not found, installing..."
        sudo apt-get update
        sudo apt-get install -y build-essential || _install_status 1 "Failed to install build tools"
    fi
    sudo gcc -O2 -o /usr/local/bin/accessd-network-monitor $webroot_dir/installers/accessd-network-monitor.c || _install_status 1 "Failed to compile accessd-network-monitor.c"
    echo "Copying accessd-network-activity@.service to /lib/systemd/system/"
    sudo cp $webroot_dir/installers/accessd-network-activity@.service /lib/systemd/system/ || _install_status 1 "Unable to move accessd-network-activity.service file"
    sudo systemctl daemon-reload
    echo "Enabling accessd-network-activity@wlan0.service"
    sudo systemctl enable accessd-network-activity@wlan0.service || _install_status 1 "Failed to enable accessd-network-activity.service"
    echo "Starting accessd-network-activity@wlan0.service"
    sudo systemctl start accessd-network-activity@wlan0.service || _install_status 1 "Failed to start accessd-network-activity.service"
    sleep 0.5
    ...
    echo "Setting ownership for ${accessd_user} on ${webroot_dir}/app/net_activity"
    sudo chown -R $accessd_user:$accessd_user $webroot_dir/app/net_activity || _install_status 1 "Unable to set ownership of ${webroot_dir}/app/net_activity"
...
function _patch_system_files() {
    ...
    # Create sudoers
    _install_log "Adding accessd.sudoers to ${accessd_sudoers}"
    sudo cp "$webroot_dir/installers/accessd.sudoers" $accessd_sudoers || _install_status 1 "Unable to apply accessd.sudoers to $accessd_sudoers"
    sudo chmod 0440 $accessd_sudoers || _install_status 1 "Unable to change file permissions for $accessd_sudoers"

    if [ ! -d "$accessd_dir/system" ]; then
        sudo mkdir $accessd_dir/system || _install_status 1 "Unable to create directory '$accessd_dir/system'"
    fi

    _install_log "Copying AccessD debug log control script"
    sudo cp "$webroot_dir/installers/"debuglog.sh "$accessd_dir/system" || _install_status 1 "Unable to move debug logging script"

    _install_log "Copying AccessD install loader"
    sudo cp "$webroot_dir/installers/"raspbian.sh "$accessd_dir/system" || _install_status 1 "Unable to move application update script"

    # Set ownership and permissions
    sudo chown -c root:root "$accessd_dir/system/"*.sh || _install_status 1 "Unable change owner and/or group"
    sudo chmod 750 "$accessd_dir/system/"*.sh || _install_status 1 "Unable to change file permissions"
...
function _optimize_php() {
    if [ "$upgrade" == 0 ]; then
        ...
        sudo cp "$phpiniconf" "$accessd_dir/backups/php.ini.$datetimephpconf"
        sudo ln -sf "$accessd_dir/backups/php.ini.$datetimephpconf" "$accessd_dir/backups/php.ini"
...
function _install_complete() {
    ...
    if [ "$repo" == "AccessD/accessd-insiders" ]; then
        echo -e "${ANSI_RASPBERRY}"
        echo "Thank you for supporting this project as an Insider!"
        echo -e "${ANSI_RESET}"
    else
        echo "Join AccessD Insiders for early access to exclusive features!"
        echo -e "${ANSI_RASPBERRY}"
        echo "> https://docs.accessd.com/insiders/"
        echo "> https://github.com/sponsors/AccessD/"
        echo -e "${ANSI_RESET}"
    fi
    if [ "$assume_yes" == 0 ]; then
