...
/**
 * A dhcpcd configuration class for AccessD
 *
...
class DhcpcdManager
{
        ...
        ): bool
    {
        ...
        if ($bridgedEnable) {
            ...
            $config[] = '';
            $config[] = '# AccessD br0 configuration';
            ...
            $config = [
                '# AccessD ' . $ap_iface . ' configuration',
                'interface ' . $ap_iface,
            ...
            $config = array_keys(getDefaultNetOpts('dhcp', 'options'));
            $config[] = '# AccessD uap0 configuration';
        ...
        if (preg_match('/wlan[3-9]\d*|wlan[1-9]\d+/', $ap_iface)) {
            ...
            if (!strpos($dhcp_cfg, 'metric')) {
                $pattern = '/^#\sAccessD\s' . preg_quote($ap_iface, '/') . '\sconfiguration\n' .
                          '(?:.*\n)*?' .
                          '(?:\n)*' .
                          '(?=#\sAccessD\s|\z)/m';
    ...
    {
        $cfg[] = '# AccessD '.$iface.' configuration';
        $cfg[] = 'interface '.$iface;
        ...
        if (!preg_match('/^interface\s'.$iface.'$/m', $dhcp_cfg)) {
            ...
            $cfg = join(PHP_EOL, $cfg);
            $pattern = '/^#\sAccessD\s' . preg_quote($iface, '/') . '\sconfiguration\n' .
                      '(?:.*\n)*?' .
                      '(?:\n)*' .
                      '(?=#\sAccessD\s|\z)/m';
    ...
    public function remove(string $iface, StatusMessage $status): bool
    {
        ...
        $modified_cfg = preg_replace('/^#\sAccessD\s'.$iface.'\s.*?(?=\s*^\s*$)([\s]+)/ms', '', $dhcp_cfg, 1);
    ...
    {
        $dhcp_cfg = preg_replace('/^#\sAccessD\s'.$iface.'\s.*?(?=\s*^\s*$)([\s]+)/ms', '', $dhcp_cfg, 1);
        return $dhcp_cfg;
    }
    ...
    */
        ...
        string $domain_name_servers): array
    {
        $dhcp_cfg = file_get_contents(self::CONF_DEFAULT);
        $existing_config = [];
        $section_regex = '/^#\sAccessD\s'.preg_quote($ap_iface, '/').'\s.*?(?=\s*^\s*$)/ms';
        ...
        $config = [ '# AccessD '.$ap_iface.' configuration' ];
    ...
    public function getInterfaceConfig(string $iface): array
    {
        ...
        if (file_exists(self::CONF_DEFAULT) && is_readable(self::CONF_DEFAULT)) {
            ...
            // match interface block starting with '# AccessD <iface> configuration'
            $sectionPattern = '/^#\sAccessD\s' . preg_quote($iface, '/') . '\sconfiguration.*?(?=^(?:#\sAccessD\s|\s*$))/ms';
