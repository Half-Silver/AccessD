 ...
 * from /etc/accessd/networking/defaults.json
 *
 * @param string $svc
 * @param string $iface
 * @return string $value
...
function validateInterface($interface)
{
    ...
    $valid_interfaces = shell_exec('ip -o link show | awk -F": " \'{print $2}\' 2>/dev/null');

    // Convert to array (one interface per line)
    $valid_interfaces = explode("\n", trim((string) $valid_interfaces));
 ...
 * From @marek-guran's material-dark theme for AccessD
